...
if ($k==1) {
	...
	<input type="hidden" id="spacestart" value="<?php echo $row['id'] ?>" />
	<ul onClick="fillstart('<li id=\'<?php echo $row['id'] ?>\' onClick=\'$(this).hide(); removestart(this.id)\'><?php echo $row['spot'] ?></li>');addstart('<?php echo $row['id'] ?>')"><?php echo $row['spot']?></ul>    
