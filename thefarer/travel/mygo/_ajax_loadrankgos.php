<?php
require("../../_config.php");
$gosql= "SELECT * FROM gos WHERE id= '".$_GET['mygo']."'"; $gorow=mysql_fetch_assoc(mysql_query($gosql)); 
for ($n=0; $n<5; $n++) {
	$ranksql= "SELECT * FROM gos_ranks WHERE 
	status!=-1 
	AND gos_id= '".$_GET['mygo']."' 
	AND rank_to>=64 
	AND subgos_id< '".$_SESSION['TMP_LOADRANKGO_96']."' 
	AND rank_to= '".$_SESSION['TMP_LOADRANKTO_96']."' 
	AND rank_time>=96
	ORDER BY rank_to DESC, id DESC limit 1";
	$rankrow=mysql_fetch_assoc(mysql_query($ranksql)); $m=96;
	if ($rankrow['id']==0) {
		$ranksql= "SELECT * FROM gos_ranks WHERE 
		status!=-1 
		AND gos_id= '".$_GET['mygo']."' 
		...
		if ($rankrow['id']==0) {
			...
			ORDER BY rank_to DESC, id DESC limit 1";
			$rankrow=mysql_fetch_assoc(mysql_query($ranksql)); $m=0;
			if ($rankrow['id']==0) {
				$ranksql= "SELECT * FROM gos_ranks WHERE 
				status!=-1 
				AND gos_id= '".$_GET['mygo']."' 
	...
	$gosql= "SELECT * FROM gos WHERE id= '".$rankrow['subgos_id']."' "; $gorow=mysql_fetch_assoc(mysql_query($gosql)); 
