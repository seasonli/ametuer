...
window.onload = function() {
  $("body").prepend('<div class="dialog"><ul class="dialog_alert"></ul><ul class="dialog_loading">加载中…</ul></div>');

  var scrollHeight = document.documentElement.clientHeight + document.body.clientHeight;
  $(".footer").css("top", scrollHeight + "px");
  $(".dialog").css("height", scrollHeight + 80 + "px");

  $(".dialog").click(function(){
    $(".dialog, .dialog_alert").hide(1);
  });
  ...
  function requestCheckAddress() { // judge if address is needed
    ...
    var requestCheckAddressUrl = "http://m.mobvoi.com/search/qa/?query=" + QUERY.query
      + "&query_type=location"
      + "&address=,,,,,,0,0"
      + "&user_id=" + QUERY.user
      + "&msg_id=" + QUERY.msg
  ...
  function requesSearch() {
    ...
    if(GEO.bd.status == true) {
      ...
      window.location.href = requestUrl;
        ...
        $(".dialog, .dialog_loading").show(1);         
        if (waitTimes >= 3) {
           $(".dialog_loading").hide(1);   
          $(".dialog_alert").show(1).html("<li class='title'>换一种方式提问</li><li>由于某些原因暂时无法获取你的位置，可能无法给你想要的答案<br/>但你可以这样像这样提问<p><b>“" + QUERY.example + "</b>”</p></li><li><div class='details'>为什么出门问问得不到你的地址<br/><br/>1. 检查你的浏览器设置及手机设置<br/>2. 在更空旷的地方或许有更好的信号<br/>3. 可能你的浏览器不支持地理信息的获取</div></li>");
        }
        else {
